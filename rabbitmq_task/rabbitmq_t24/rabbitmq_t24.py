...
class RabbitmqConnectionTask24:
    ...
    async def connect(self):
        ...
        self.retry_exchange = await self.channel.declare_exchange(
            name="retry_exchange_task24",
            type=aio_pika.ExchangeType.DIRECT,
            durable=True,
        )
        for i in range(self.attempts):
            queue = await self.channel.declare_queue(
                name=f"retry_{i}_task24",
                durable=True,
                arguments={
                    "x-message-ttl": 5000 * (2 ** i),
                    "x-dead-letter-exchange": "main_exchange_task24",
                    "x-dead-letter-routing-key": "main_task24.key",
                },
            )
            await queue.bind(
                exchange=self.retry_exchange,
                routing_key=f"retry_{i}_task24.key"
            )
            print(f"Retry {i} attempt queue is connected...")
        #--------------------
        #   Retry Exchange
        #--------------------
        self.dlq_exchange = await self.channel.declare_exchange(
            name="dlq_exchange_task24",
            type=aio_pika.ExchangeType.DIRECT,
            durable=True,
        )
        ...
        print("Rabbitmw Task24 is connected...")


    async def close(self):
        if self.connection:
            await self.connection.close()
            print("Rabbitmq Task24 is closed...")
