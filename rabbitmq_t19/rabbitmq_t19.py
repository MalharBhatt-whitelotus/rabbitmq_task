...
class RabbitmqConnectionTask19:
    ...
    async def connect(self):
        ...
        self.main_exchange = await self.channel.declare_exchange(
            name="main_exchange_task19",
            type=aio_pika.ExchangeType.DIRECT,
            durable=True,
        )
        self.main_queue = await self.channel.declare_queue(
            name="main_queue_task19",
            durable=True,
        )
        await self.main_queue.bind(
            exchange=self.main_exchange, routing_key="main_task19.key",
        )
        ...
        self.retry_exchange = await self.channel.declare_exchange(
            name="retry_exchange_task19",
            type=aio_pika.ExchangeType.DIRECT,
            durable=True
        )
        self.retry_queue = await self.channel.declare_queue(
            name="retry_queue_task19",
            durable=True,
            arguments={
                "x-message-ttl": 5000,
                "x-dead-letter-exchange": "main_exchange_task19",
                "x-dead-letter-routing-key": "main_task19.key"
            },
        )
        ...
        print("Rabbitmq task19 is connected...")
